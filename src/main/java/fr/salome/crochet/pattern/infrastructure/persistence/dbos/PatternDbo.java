...
public class PatternDbo {
	...
	public PatternDbo(UUID id, String name, GaugeDbo gauge, List<String> instructions) {
		this.id = id;
		this.name = name;
		this.gauge = gauge;
	...
	public static PatternDbo fromDomain(Pattern pattern) {
		return new PatternDbo(pattern.id().value(), pattern.name(), GaugeDbo.fromDomain(pattern.gauge()), pattern.instructions());
	}

	public static Pattern toDomain(PatternDbo dbo) {
		return new Pattern(dbo.getId(), dbo.getName(), dbo.getInstructions(), GaugeDbo.toDomain(dbo.getGauge()));
	}
}
